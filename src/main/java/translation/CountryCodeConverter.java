    ...
    public CountryCodeConverter(String filename) {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                String country = "";
                int CountryCodeIndex = parts.length - 3;
                int ThreeLetterIndex = parts.length - 2;

                for (int i = 0; i < CountryCodeIndex; i++){
                    country = country + parts[i];
                    if (i + 1 < CountryCodeIndex){
                        country += " ";
                    }
                }
                countryCodeToCountry.put(parts[ThreeLetterIndex].toLowerCase(), country);
                countryToCountryCode.put(country, parts[ThreeLetterIndex].toLowerCase());
            }
        }
