...
public class JSONTranslator implements Translator {
    ...
    public JSONTranslator(String filename) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                Map<String, String> countrylang = new HashMap<>();
                while (keys.hasNext()) {
                    String key = keys.next();
                    if ("id".equals(key) || "alpha2".equals(key) || "alpha3".equals(key)) {
                        continue;
                    }
                    if (countryObject.get(key) instanceof String) {
                        languages.add(key);
                        countrylang.put(key, countryObject.getString(key));
                    }
                }
                countryLanguages.put(country, languages);
                translations.put(country, countrylang);
            }
        }
    ...
    public String translate(String country, String language) {
        if (translations.get(country).get(language) == null) {
            return null;
        }
